const links = {
  "국어": "zoommtg://zoom.us/join?action=join&confno=3300965795&pwd=dimigo",
  "국어 - 김태철": "zoommtg://zoom.us/join?action=join&confno=8134411742&pwd=dimigo",
  "영어 - 공정도": "zoommtg://zoom.us/join?action=join&confno=4700404365&pwd=dimigo",
  "영어 - 전유원": "zoommtg://zoom.us/join?action=join&confno=7385237525&pwd=dimigo",
  "수학 - 류명희": "zoommtg://zoom.us/join?action=join&confno=6838770867&pwd=dimigo",
  "수학 - 이효현": "zoommtg://zoom.us/join?action=join&confno=7987833794&pwd=dimigo",
  "사회": "https://bit.ly/34rBBfM",
  "과학": "zoommtg://zoom.us/join?action=join&confno=7178015132&pwd=dimigo",
  "역사": "zoommtg://zoom.us/join?action=join&confno=3955689232&pwd=dimigo",
  "상경": "zoommtg://zoom.us/join?action=join&confno=3154650453&pwd=dimigo",
  "컴그": "zoommtg://zoom.us/join?action=join&confno=7987737883&pwd=dimigo",
  "컴일": "zoommtg://zoom.us/join?action=join&confno=6971690768&pwd=dimigo",
  "자료구조 A": "zoommtg://zoom.us/join?action=join&confno=433%202940405&pwd=dimigo",
  "자료구조 B": "zoommtg://zoom.us/join?action=join&confno=4679526373&pwd=py2020",
  "음악": "https://bit.ly/34rBBfM",
  "체육": "zoommtg://zoom.us/join?action=join&confno=7760965125&pwd=dimigo",
  "진로": "zoommtg://zoom.us/join?action=join&confno=8448373394&pwd=dimigo",
  "미술": "zoommtg://zoom.us/join?action=join&confno=8448373394&pwd=dimigo",
  "CA": "",
  "HR": "",
};
const schedule = {
  "1-1": {
    "Mon": ['사회', '음악', '영어 - 공정도', '역사', '컴그', '컴그', '수학 - 이효현'],
    "Tue": ['상경', '사회', '국어 - 김태철', '과학', '체육', 'CA', 'CA'],
    "Wed": ['영어 - 공정도', '상경', '음악', '역사', '과학', '수학 - 이효현', ''],
    "Thu": ['영어 - 공정도', '상경', '음악', '역사', '과학', '국어', 'HR'],
    "Fri": ['수학 - 이효현', '영어 - 공정도', '역사', '과학', '국어', '음악', '진로'],
    "Sat": ['No Schedule'],
    "Sun": ['No Schedule']
  },
  "1-2": {
    "Mon": ['역사', '사회', '수학 - 류명희', '영어 - 공정도', '미술', '미술', '상경'],
    "Tue": ['국어', '컴그', '컴그', '사회', '과학', 'CA', 'CA'],
    "Wed": ['과학', '국어', '체육', '상경', '컴그', '컴그', ''],
    "Thu": ['사회', '상경', '영어 - 공정도', '역사', '수학 - 류명희', '미술', 'HR'],
    "Fri": ['수학 - 류명희', '과학', '국어 - 김태철', '진로', '영어 - 공정도', '체육', '역사'],
    "Sat": ['No Schedule'],
    "Sun": ['No Schedule']
  },
  "1-3": {
    "Mon": ['영어 - 공정도', '자료구조 A', '자료구조 A', '음악', '자료구조 B', '자료구조 B', '수학 - 류명희'],
    "Tue": ['수학 - 류명희', '과학', '체육', '음악', '컴일', '진로', 'CA'],
    "Wed": ['자료구조 A', '영어 - 공정도', '진로', '국어', '사회', '수학 - 류명희'],
    "Thu": ['컴일', '과학', '사회', '국어', '음악', '체육', 'HR'],
    "Fri": ['국어', '사회', '자료구조 B', '컴일', '자료구조 A', '과학', '영어 - 공정도'],
    "Sat": ['No Schedule'],
    "Sun": ['No Schedule']
  },
  "1-4": {
    "Mon": ['진로', '자료구조 B', '음악', '국어', '사회', '수학 - 류명희', '과학'],
    "Tue": ['자료구조 B', '자료구조 B', '영어 - 공정도', '컴일', '국어', 'CA', 'CA'],
    "Wed": ['수학 - 류명희', '체육', '자료구조 A', '사회', '영어 - 공정도', '과학', ''],
    "Thu": ['음악', '국어', '과학', '컴일', '자료구조 A', '자료구조 A', 'HR'],
    "Fri": ['사회', '컴일', '음악', '영어 - 공정도', '체육', '수학 - 류명희', '자료구조 A'],
    "Sat": ['No Schedule'],
    "Sun": ['No Schedule']
  },
  "1-5": {
    "Mon": ['자료구조 B', '과학', '사회', '수학 - 이효현', '자료구조 A', '체육', '국어'],
    "Tue": ['컴일', '미술', '미술', '영어 - 공정도', '수학 - 이효현', 'CA', 'CA'],
    "Wed": ['사회', '컴일', '과학', '미술', '체육', '국어', ''],
    "Thu": ['진로', '자료구조 A', '자료구조 B', '자료구조 B', '과학', '영어 - 공정도', 'HR'],
    "Fri": ['자료구조 A', '자료구조 A', '수학 - 이효현', '사회', '컴일', '국어 - 김태철', '영어 - 전유원'],
    "Sat": ['No Schedule'],
    "Sun": ['No Schedule']
  },
  "1-6": {
    "Mon": ['수학', '미술', '국어', '과학', '체육', '영어 - 공정도', '자료구조 A'],
    "Tue": ['사회', '자료구조 A', '자료구조 A', '국어', '진로', 'CA', 'CA'],
    "Wed": ['자료구조 B', '자료구조 B', '사회', '자료구조 A', '컴일', '체육', ''],
    "Thu": ['영어 - 공정도', '컴일', '미술', '미술', '수학 - 이효현', '과학', 'HR'],
    "Fri": ['과학', '수학 - 이효현', '국어', '자료구조 B', '사회', '영어 - 전유원', '컴일'],
    "Sat": ['No Schedule'],
    "Sun": ['No Schedule']
  }
};
const days = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
let className = '';
let todaySchedule = [];
let recentName = '';

document.addEventListener("DOMContentLoaded", () => {
  initialize();
});

const initialize = () => {
  if (localStorage.getItem('class') == null) {
    document.getElementsByTagName('h2')[0].innerHTML = '반을 선택해주세요.';
    return;
  }
  className = localStorage.getItem('class');
  let elements = document.getElementsByTagName('option');
  if (elements[0].value == '') {
    classSelector.remove(0);
  }
  for (let i = 0; i < elements.length; i++) {
    if (className == elements[i].value) {
      classSelector.selectedIndex = i;
    }
  }
  loop();
};

const loop = () => {
  setTimeout(loop, 1000);
  if (className == "1-4") {
    links["CA"] = "https://bit.ly/2Qf4l33";
    links["HR"] = "https://bit.ly/2Qf4l33";
  } else if (className == "1-3") {
    links["CA"] = links["자료구조 A"];
    links["HR"] = links["자료구조 A"];
  } else if (className == "1-1") {
    links["CA"] = links["영어 - 공정도"];
    links["HR"] = links["영어 - 공정도"];
  } else if (className == "1-2") {
    links["CA"] = links["수학 - 류명희"];
    links["HR"] = links["수학 - 류명희"];
  } else if (className == "1-5") {
    links["CA"] = links["음악"];
    links["HR"] = links["음악"];
  } else if (className == "1-6") {
    links["CA"] = links["사회"];
    links["HR"] = links["사회"];
  }
  const d = new Date();
  todaySchedule = schedule[className][days[d.getDay()]];
  if (todaySchedule[0] == 'No Schedule') {
    noSchedule();
    return;
  }
  let elements = document.getElementsByClassName('schedule');
  for (let i = 0; i < 7; i++) {
    elements[i].textContent = todaySchedule[i];
    elements[i].classList.remove('selected');
  }
  let hour = d.getHours();
  let min = d.getMinutes();

  // 수업 전
  if (hour < 8) {
    document.getElementsByTagName('h2')[0].innerHTML = '아직 리잠 잘 시간이야..';
  }

  // 조례
  else if (hour <= 8 && min < 45) {
    updateH2('조례', links.HR, true);
    redirect(links.HR, '조례');
  } else if (hour == 8 && min < 58) {
    updateH2('조례', links.HR, false);
    redirect(links.HR, '조례');
  }

  // 연결
  else if (hour == 9) { // 1교시 연결
    redirect(links[todaySchedule[0]], todaySchedule[0]);
  }
  else if (hour == 9 && min == 55) {  // 2교시 연결
    redirect(links[todaySchedule[1]], todaySchedule[1]);
  }
  else if (hour == 10 && min == 55) { // 3교시 연결
    redirect(links[todaySchedule[2]], todaySchedule[2]);
  }
  else if (hour == 11 && min == 55) {
    redirect(links[todaySchedule[3]], todaySchedule[3]);
  }
  else if (hour == 13 && min == 45) {
    redirect(links[todaySchedule[4]], todaySchedule[4]);
  }
  else if (hour == 14 && min == 45) {
    redirect(links[todaySchedule[5]], todaySchedule[5]);
  }
  else if (hour == 15 && min == 45) {
    redirect(links[todaySchedule[6]], todaySchedule[6]);
  }


  // 1교시
  if (hour == 8 && min >= 0 && min < 50) { // 8시 0분 ~ 50분
    elements[0].classList.add('selected');
    updateH2(todaySchedule[0], links[todaySchedule[0]], false); // 1번째 주소, 지금 수업 중
  }

  // 1교시 쉬는시간
  else if (hour == 8 && min >= 50 && min < 60) {
    updateH2(todaySchedule[1], links[todaySchedule[1]], ture); // 2번째 주소, 다음 수업 준비
  }

  // 2교시
  else if (hour == 9 && min >= 0 && min < 50) { // 9시 0분 ~ 50분
    elements[1].classList.add('selected');
    updateH2(todaySchedule[1], links[todaySchedule[1]], false); // 2번째 주소, 지금 수업 중
  }

  // 2교시 쉬는시간
  else if (hour == 9 && min >= 50 && min < 60) {
    updateH2(todaySchedule[2], links[todaySchedule[2]], ture); // 3번째 주소, 다음 수업 준비
  }

  // 3교시
  else if (hour == 10 && min >= 0 && min < 50) { // 10시 0분 ~ 50분
    elements[2].classList.add('selected');
    updateH2(todaySchedule[2], links[todaySchedule[2]], false); // 3번째 주소, 지금 수업 중
  }

  // 3교시 쉬는시간
  else if (hour == 10 && min >= 50 && min < 60) {
    updateH2(todaySchedule[3], links[todaySchedule[3]], ture);
  }

  // 4교시
  else if (hour == 11 && min >= 0 && min < 50) {
    elements[3].classList.add('selected');
    updateH2(todaySchedule[3], links[todaySchedule[3]], false);
  }

  // 점심시간
  else if (hour == 12 || (hour == 13 && min < 40)) {
    document.getElementsByTagName('h2')[0].innerHTML = '점심시간 !';
  }

  // 5교시
  else if ((hour == 13 && min >= 50 && min < 60) || (hour == 14 && min >= 0 && min < 40)) {
    elements[4].classList.add('selected');
    updateH2(todaySchedule[4], links[todaySchedule[4]], false);
  }

  // 5교시 쉬는시간
  else if (hour == 14 && min >= 40 && min < 50) {
    updateH2(todaySchedule[5], links[todaySchedule[5]], ture);
  }

  // 6교시
  else if ((hour == 14 && min >= 50 && min < 60) || (hour == 15 && min >= 0 && min < 40)) {
    elements[5].classList.add('selected');
    updateH2(todaySchedule[5], links[todaySchedule[5]], false);
  }

  // 6교시 쉬는시간
  else if (hour == 15 && min >= 40 && min < 50) {
    updateH2(todaySchedule[6], links[todaySchedule[6]], ture);
  }

  // 7교시
  else if ((hour == 15 && min >= 50 && min < 60) || (hour == 16 && min >= 0 && min < 40)) {
    elements[6].classList.add('selected');
    updateH2(todaySchedule[6], links[todaySchedule[6]], false);
  }

  // 종례
  else if (hour == 15 && min < 45) {
    updateH2('종례', links.HR, true);
    redirect(links.HR, '종례');
  }
  else if (hour == 15) {
    updateH2('종례', links.HR, false);
    redirect(links.HR, '종례');
  } else {
    document.getElementsByTagName('h2')[0].innerHTML = '수업끝 !!';
    recentName = '';
  }
};

const classSelected = e => {
  localStorage.setItem('class', e.value);
  initialize();
};

const updateH2 = (className, classLink, isReady) => {
  document.getElementsByTagName('h2')[0].innerHTML = `${isReady ? '다음 수업 준비' : '지금 수업중'} : <span>${className}</span>, <a href="${classLink}" target="_blank">여기</a>를 눌러 수동접속`;
};

const noSchedule = () => {
  document.getElementsByTagName('h2')[0].innerHTML = '오늘은 수업이 없는 날입니다!';
};

const redirect = (link, name) => {
  if (recentName == name) return;
  let a = document.createElement("a");
  a.href = link;
  if (link.indexOf("zoommtg") == -1) {
    a.target = "_blank";
  }
  a.click();
  recentName = name;
}
